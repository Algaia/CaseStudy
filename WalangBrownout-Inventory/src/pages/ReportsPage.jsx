...
export default function ReportsPage({ products, lots, activity, writeoffs, onExport }) {
  const latestActivity = activity[0]?.time || 'No activity recorded';
  const reports = [
    { id: 'inventory', title: 'Inventory balance report', detail: 'On-hand, available, reorder points, and storage locations for all active products.', type: 'CSV', icon: 'boxes', updated: `Updated ${latestActivity}` },
    { id: 'expiry', title: 'Expiry and FEFO report', detail: 'Filter lots ranked by expiry date, location, and pick priority.', type: 'CSV', icon: 'calendar', updated: `Updated ${latestActivity}` },
    ...
    { id: 'activity', title: 'Inventory activity audit', detail: 'Traceable timeline of stock, picking, and purchasing events.', type: 'CSV', icon: 'clock', updated: `Updated ${latestActivity}` },
  ];
  const sourceFor = (id) => id === 'inventory' ? products : id === 'expiry' ? lots : id === 'writeoff' ? writeoffs : id === 'activity' ? activity : [];
  return <>
    ...
    <SectionCard title="Available exports" subtitle="CSV exports open in Excel, Google Sheets, or any accounting workflow.">
      <div className="report-grid">{reports.map((report) => { const updated = report.id === 'writeoff' && writeoffs.length ? `${writeoffs.length} record(s) logged` : report.updated; const emptyWriteoffs = report.id === 'writeoff' && writeoffs.length === 0; return <article className="report-card" key={report.id}><span className="report-icon"><Icon name={report.icon} size={22} /></span><div><StatusPill tone="neutral">{report.type}</StatusPill><h3>{report.title}</h3><p>{report.detail}</p></div><footer><span className={updated.startsWith('No') ? 'muted' : 'fresh-dot'}>{updated}</span><Button variant="secondary" icon="download" disabled={emptyWriteoffs} onClick={() => onExport(report.id, sourceFor(report.id))}>{emptyWriteoffs ? 'No records' : 'Download'}</Button></footer></article>; })}</div>
    </SectionCard>
