...
import { formatNumber, seasonalOutlook } from '../data';
import { Button, Icon, MetricCard, PageTitle, SectionCard, StatusPill } from '../components/ui';

export default function ReorderPage({ recommendations, onCreatePO, onExport }) {
  ...
  const projectedCoverage = recommendations.length ? Math.round((recommendations.filter((item) => item.suggestedOrder === 0 || item.created).length / recommendations.length) * 100) : seasonalOutlook.projectedCoveragePercent;
  return <>
    <PageTitle eyebrow="PROCUREMENT" title="Reorder recommendations" description="Recommendations combine live stock, supplier lead time, safety stock, and seasonality." action={<Button variant="secondary" icon="download" onClick={() => onExport('reorder-plan', recommendations)}>Export plan</Button>} />
    <section className="metrics-grid reorder-metrics"><MetricCard label="Recommendations" value={recommendations.filter((item) => item.suggestedOrder > 0).length} helper="require procurement action" tone="amber" icon="warning" /><MetricCard label="Peak season" value={`${seasonalOutlook.peakSeasonWeeks} weeks`} helper="until portable AC demand rises" tone="violet" icon="calendar" /><MetricCard label="Projected coverage" value={`${projectedCoverage}%`} helper="already covered or ordered" tone="green" icon="trend" /></section>
