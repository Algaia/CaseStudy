...
export default function AlertsPage({ alerts, onRead, onReadAll, setCurrentPage }) {
  ...
  return <>
    ...
    <SectionCard title="All alerts" subtitle={`${alerts.filter((alert) => !alert.read).length} items still need review.`}>
      <div className="alert-tabs">{[['All', 'All alerts'], ['critical', 'Low stock'], ['expiry', 'Expiry (FEFO)'], ['seasonal', 'Seasonal'], ['mismatch', 'Mismatch']].map(([id, label]) => <button className={filter === id ? 'active' : ''} onClick={() => setFilter(id)} key={id}>{label}{id === 'All' && <span>{alerts.length}</span>}</button>)}</div>
